...
#link(<sec-retract-definition>)[上一节]把收缩写成一张图表. 幂等态射也可以这样处理: 用一个很小的范畴, 记录一个对象, 一条自态射, 以及这条自态射必须满足的关系.
...
这个例子说明, 幂等关系本身并不保证所需的收缩对象已经在范畴里. #link(<sec-projective-retracts>)[最后一节]给出一个重要的代数实例: 从自由模出发, 加入它们的收缩对象, 就得到了投射模.

== 投射模与自由模的收缩 <sec-projective-retracts>
