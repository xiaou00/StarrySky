...
后面遇到谱及环谱的模时, 这些规则可以直接使用: 用纤维描述零化条件, 用余纤维构造同伦商, 用移位把它们互相转换. #link(<sec-spectrum-definitions>)[下一节]的谱范畴 $Sp$, 就是把这种稳定的运算方式引入生象所得到的基本例子.

== 谱的两种定义 <sec-spectrum-definitions>
...
=== 两种定义怎样互相恢复 <sec-spectrum-equivalence>
...
=== 最简单的例子与使用方式 <sec-spectrum-examples>
...
直观上, 从 $x,Sigma x,Sigma^2 x,dots$ 出发, 把"继续悬挂"变成可逆操作, 就得到 $Sigma^oo x$. 这列空间先给出一个*预谱*, 再经过谱化得到#link(<sec-spectrum-definitions>)[谱的定义一节]意义下的谱; 原来的结构映射不必已经满足 $Omega$-谱条件.
...
#example(title: "把普通上同调放进一个谱")[
  ...
  #link(<sec-spectrum-examples>)[前面定义的 $H A$] 把这些目标空间组织在一起. 由悬挂谱伴随, 对所有整数 $n$,
...
#definition(title: "砸积与同调谱")[
  ...
  对有限带基点生象 $x$, 这也就是#link(<sec-spectrum-equivalence>)[两种谱定义的等价]中构造的延拓 $L_E (x)$: 按 $x$ 的胞腔粘合方式, 用 $E$ 代替 $S^0$ 来构造一个谱.
...
#definition(title: "算筹作用的含义")[
  这里使用单色的无穷算筹: 它为每个 $r>=0$ 给出一个 $r$ 输入运算的生象 $cal(O)(r)$, 并带有代入, 输入置换与恒等运算, 以及它们的全部相容性.

  对谱 $A$, 其*自同态算筹*的运算生象为
  $ op("End")_A (r):=Map_(Sp) (A^(smash r),A), quad A^(smash 0):=SS $
  代入由复合与砸积给出. 一个 $cal(O)$-代数结构就是无穷算筹态射 $cal(O)->op("End")_A$: 它将每种抽象运算实现为 $A$ 上的运算, 并保持全部相容数据. 参见 @Lur17[第 2.1.1--2.1.3 节].
]
...
最后, 虽然底层的 $Sp$ 稳定, 环谱范畴本身并不稳定: 它的始对象是 $SS$, 终对象是零环谱, 两者不等价. 固定一个环谱以后, 对它的模再做线性代数, 才会重新进入稳定环境; 这正是#link(<sec-ring-spectrum-modules>)[下一节]要介绍的内容.

== 环谱的模 <sec-ring-spectrum-modules>
...
=== 相对张量积 <sec-relative-tensor-spectra>
...
以下固定交换环谱 $R$, 不要求连通. 记 $CAlg_R:=CAlg(Sp)_(R \/)$, 其对象是交换环谱 $A$ 连同 $R->A$. 对交换 $R$-代数, $A⊗_R B$ 表示推出 $A coprod_R B$; 它的底层模正是#link(<sec-relative-tensor-spectra>)[前面介绍的相对张量积].
...
右边是 $Omega^oo A$ 中对应 $(pi_0 A)^times$ 的连通分支. 后面从点函子看, $R[t]$ 因而给出仿射直线, 而 $R[t,t^(-1)]$ 给出其中的乘法群. 在普通零次环上, 局部化对应主开集 $D(f) subset.eq Spec(pi_0 R)$; 各个 $pi_i R$ 都在同一主开集上局部化. 这正是#link(<sec-perfect-vector-bundles>)[向量丛一节]用局部化定义局部自由模的依据.
