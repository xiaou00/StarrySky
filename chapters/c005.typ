...
#definition(title: "零对象与带基点无穷范畴")[
  若 $0 in C$ 同时是始对象和终对象, 则称它为*零对象*. 也就是说, 对每个 $x in C$,
  $ Map_C (0,x) tilde.eq *, quad Map_C (x,0) tilde.eq * $
  存在零对象的无穷范畴称为*带基点的* (pointed). 此时复合 $x->0->y$ 给出*零态射*, 也为每个 $Map_C (x,y)$ 指定基点.
]

例如, 带基点生象组成无穷范畴 $Ani_*$: 对象是生象连同一个指定点, 态射保持基点, 并保留相应的同伦数据. 单点生象 $*$ 是它的零对象. 因而这里的"零"可以是一个点, 并不表示空对象.
...
#definition(title: "纤维与余纤维")[
  ...
  $ fib(f)->x stretch(->)^f y, quad x stretch(->)^f y->cofib(f) $
  第一条称为*纤维序列*, 第二条称为*余纤维序列*. 序列还包含相邻两箭头复合到零态射的指定同伦.
] <def-stable-fiber-cofiber>

纤维是在问"哪些东西被 $f$ 送到了零, 又是怎样变成零的". 例如在 $Ani_*$ 中, 它的一个点由 $a in x$ 和一条从 $f(a)$ 到基点的路径组成. 余纤维则把 $x$ 沿 $f$ 接到 $y$ 上的那部分压到零, 同时记住压缩过程; 在空间模型里, 可以通过给 $f$ 接上一个锥来理解. 所以它们分别是保留同伦信息的"核"与"商".
...
推出描述"怎样粘合", 拉回描述"怎样满足相容条件". 稳定性把这两种构造联系起来. 特别地, 在带有指定零同伦的序列 $x->y->z$ 中, "由 $x->y$ 取余纤维得到 $z$"与"由 $y->z$ 取纤维得到 $x$"是同一件事. 我们于是可以统一称它为*正合序列*.
...
#definition(title: "谱: 切除函子定义")[
  一个*谱* (spectrum) 是约化切除函子 $F:Ani_*^"fin"->Ani_*$. 记它们组成的满子无穷范畴为
  $ Sp_"exc":=Exc_* (Ani_*^"fin",Ani_*) subset.eq Fun(Ani_*^"fin",Ani_*) $
...
#definition(title: "映射谱 / Mapping spectrum")[
